...
from .data_splits.newDatasplit import DataSplitter
...
#         print('all_kg: ', all_kg)
        test_kg['split'] = 'test'
#         all_kg.to_csv('data/test_allkg.csv')
#         test_kg.to_csv('data/test_testkg.csv')
        
        nodeMap = dict(pd.read_csv('data/node.csv', sep='\t')[['node_id', 'node_index']].astype(str).values)
        all_kg['x_index'] = all_kg.x_id.map(nodeMap).astype(int)
        all_kg['y_index'] = all_kg.y_id.map(nodeMap).astype(int)
...
def process_disease_area_split(data_folder, df, df_test, split):
    disease_file_path = os.path.join('txgnn', 'disease_files')
#     print(disease_file_path)
