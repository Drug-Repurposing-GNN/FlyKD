...
#                 g_neg_psuedo = neg_sampler(g_pos_psuedo)

#                 print([len(v[0]) for k, v in  self.additional_train.items()])
#                 # print('--------------------')
#                 # print(psuedo_negative_graph)
#                 # print('--------------------')
#                 # print(g_psuedo_labels)

#                 psuedo_dd_etypes = [('drug', 'contraindication', 'disease'), 
#                                     ('drug', 'indication', 'disease'), 
#                                     ('disease', 'rev_contraindication', 'drug'), 
#                                     ('disease', 'rev_indication', 'drug'),]
#                 # psuedo_score_neg = [pred_score_neg[i] for i in psuedo_dd_etypes]
#                 # print(self.additional_train)
#                 # print(self.G)
#                 # print(len(psuedo_neg[0]))
#                 # print(len(psuedo_neg[1]))
#                 # indication_size = len(psuedo_score_neg[1])
#                 # contraindication_size = len(psuedo_score_neg[0])
#                 # print(indication_size, contraindication_size)
#                 # rand_ind = [random.randint(indication_size) for _ in range(self.psuedo_size)]
#                 # rand_c_ind = [random.randint(contraindication_size) for _ in range(self.psuedo_size)]
#                 # psuedo_random_neg = torch.cat(psuedo_score_neg[0], psuedo_score_neg[1])
#                 # print(psuedo_random_neg.shape)
#                 # print(psuedo_random_neg.device)
...
#                 ###
#                 psuedo_pos_scores_rel, psuedo_neg_scores_rel, _, _, beta_kl_loss = self.model(self.G, g_neg_psuedo, g_pos_psuedo, psuedo=True) ## psuedo skips on off-label dd relation. 
#                 # _, psuedo_neg_scores_rel, _, psuedo_neg_scores, _ = self.model(self.G, psuedo_negative_graph, psuedo=True) ## psuedo skips on off-label dd relation. 
#                 # psuedo_pos_scores, psuedo_neg_scores, _, _, _ = self.model(self.G, g_psuedo_labels, psuedo=True) ## psuedo skips on off-label dd relation. 
#                 # psuedo_neg_scores = torch.tensor([0], device=self.device)

#                 psuedo_pos_scores = torch.cat([psuedo_pos_scores_rel[i] for i in psuedo_dd_etypes])
#                 psuedo_neg_scores = torch.cat([psuedo_neg_scores_rel[i] for i in psuedo_dd_etypes])
#                 # _, _, _, psuedo_neg_scores, _ = self.model(self.G, psuedo_negative_graph, psuedo=True)
#                 print(len(psuedo_pos_scores))
#                 print(len(psuedo_neg_scores))
# #                 loss = loss + F.binary_cross_entropy(torch.sigmoid(psuedo_scores), torch.sigmoid(scores))
#                 psuedo_scores = torch.cat([psuedo_pos_scores, psuedo_neg_scores])
#                 print(psuedo_scores.shape)
#                 psuedo_labels = torch.cat([torch.ones(psuedo_pos_scores.shape, device=self.device), torch.zeros(psuedo_neg_scores.shape, device=self.device)])
#                 psuedo_loss = F.binary_cross_entropy(torch.sigmoid(psuedo_scores), psuedo_labels) + beta_kl_loss
